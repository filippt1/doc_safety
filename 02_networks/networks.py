...
# class for the dncnn model based on the original paper
class DnCNN(Model):
    def __init__(self, in_nc=3, out_nc=3, nc=64, nb=17):
        ...
        # stacked body layers (conv + bn + relu)
        self.body_layers = []
        for _ in range(nb - 2):
...
# class for the residual block used in drunet
class ResBlock(layers.Layer):
    def __init__(self, nc):
        super(ResBlock, self).__init__()
        # two convolutional layers with relu activation in between
...
# class for the drunet model based on the original paper
class DRUNet(Model):
    def __init__(self, in_nc=3, out_nc=3, nc=[64, 128, 256, 512], nb=4):
        ...
        # encoder (downsampling) made of resblocks and strided convs
        ...
        # decoder (upsampling) made of transposed convs, resblocks and fusion convs (to reduce channels after
    ...
    def call(self, x, training=False):
        ...
        d2 = self.up2_fusion(d2)  # squash to get correct number of channels
...
# class for the pix2pix model based on the original paper and TF implementation
class Pix2Pix(Model):
    ...
    # downsample blocks (conv + bn + leakyrelu)
    ...
    # upsample blocks (convtranspose + bn + dropout + relu)
    ...
    def build_generator(self, in_nc, out_nc):
        ...
        # convtranspose to get to output channels
    ...
    # discriminator model (downsample + conv + leakyrelu + conv)
    ...
    # training step for generator and discriminator
